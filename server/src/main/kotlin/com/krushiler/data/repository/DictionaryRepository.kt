...
class DictionaryRepository(private val dictionaryDao: DictionaryDao, private val userDao: UserDao) {
    suspend fun getUserDictionaries(user: UserDto): List<DictionaryInfoDto> {
        val dictionaries = dictionaryDao.getUserDictionaries(user.name)
        return dictionaries.map { DictionaryInfoDto(it.id, user, it.name, it.description) }
    }
